let burger = [
    {
        name: "Veggie mushroom black burger",
        descr: "Mixed green salad, Tomatoes, Edamame, Mushrooms",
        price: 16.9,
        addet: 0,
        img: "./assets/img/burger1.jpg",
    },
    {
        name: "All meat burger",
        descr: "Beef, Bacon, Dill pickles, Smoked cheese, Ketchup, BBQ souse",
        price: 15.9,
        addet: 0,
        img: "./assets/img/burger2.jpg",
    },
    {
        name: "Beef red burger",
        descr: "Beef, Cheese, Tomatoes, Lettuce, Onion",
        price: 14.9,
        addet: 0,
        img: "./assets/img/burger3.jpg",
    },
    {
        name: "Big chicken burger",
        descr: "Chicken, Cheese, Tomatoes, Lettuce, Onion, Bell pepper, ",
        price: 15.9,
        addet: 0,
        img: "./assets/img/burger4.jpg",
    },
];
let pizza = [
    {
        name: "Pizza Margherita",
        descr: "Tomato Sauce, Mozzarella",
        price: 11.9,
        addet: 0,
        img: "./assets/img/pizza1.jpg",
    },
    {
        name: "Pizza Chorizo",
        descr: "Tomato slices, Mozzarella, Chorizo",
        price: 13.9,
        addet: 0,
        img: "./assets/img/pizza2.jpg",
    },
    {
        name: "Pizza Funghi",
        descr: "Red onion, Olives, Button Mushrooms, Mozzarella",
        price: 12.9,
        addet: 0,
        img: "./assets/img/pizza3.jpg",
    },
    {
        name: "Pizza Quattro Formaggi with Chicken ",
        descr: "Chicken, Mozzarella, Gorgonzola, Fontina,  Parmigiano Reggiano",
        price: 15.9,
        addet: 0,
        img: "./assets/img/pizza4.jpg",
    },
];
let salad = [
    {
        name: "Warm beef arugula salad",
        descr: "Beef, Arugula, Field salad, Greek feta, Cherry tomatoes, Sun-dried Tomatoes, Balsamic-vinegar dressing",
        price: 16.9,
        addet: 0,
        img: "./assets/img/salad1.jpg",
    },
    {
        name: "Mini green Salad",
        descr: "Green salad, Cucumber, Carrots, Parsley, Radishes ",
        price: 7.9,
        addet: 0,
        img: "./assets/img/salad2.jpg",
    },
    {
        name: "Green Salad with sea food",
        descr: "Mixed greens, Cherry tomatoes, Red onion, Mussels, Squid rings, Shrimp, Dijon mustard-lemon dressing with dill",
        price: 16.9,
        addet: 0,
        img: "./assets/img/salad3.jpg",
    },
    {
        name: "Vegan green salad with tofu",
        descr: "Green salad, Cherry tomatoes, Cucumber, Baby spinach, Edamame, Radishes, Bittercress, Tofu, Peanuts",
        price: 14.9,
        addet: 0,
        img: "./assets/img/salad4.jpg",
    },
];
let basket = [];

let dialogRef;
let wrapperRef;
